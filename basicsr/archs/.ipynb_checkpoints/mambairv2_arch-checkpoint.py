...
class ASSM(nn.Module):
    def __init__(self, dim, d_state, input_resolution, num_tokens=64, inner_rank=128, mlp_ratio=2.,**kwargs):
        ...
        self.register_buffer("_rev1", None, persistent=False)
        self.register_buffer("_rev2", None, persistent=False)
        self.register_buffer("_cache_hw", torch.tensor([-1, -1]), persistent=False)

        # 两路序列融合层（延迟创建，首次前向根据通道数自动构建）
        self.mix_proj = None
        # === 新增结束 ===
        
        # === 新增：融合模式与节奏控制 ===
        self.fusion = str(self.scan_opts.get("fusion", "channel")).lower()  # "channel" | "temporal"
        self.fuse_steps = tuple(self.scan_opts.get("fuse", []))             # e.g. [2, 1]
    ...
    def forward(self, x, x_size, token):
        ...
        if self.dual_scan:
            ...
            if (self._cache_hw[0] != H) or (self._cache_hw[1] != W) or (self._idx1 is None):
                idx1 = build_csh_indices(
                    H, W,
                    theta_idx, D_idx,
                    stripe=self.scan_opts.get("stripe", 8),
            ...
            if (self.fusion == "temporal") and (len(self.fuse_steps) == 2):
                # ★ 时域交错融合：把两路“索引”交错成一条新索引，按 [m, n] 节奏
                m, n = int(self.fuse_steps[0]), int(self.fuse_steps[1])
                idx_csh_1d = self._idx1[0]  # (N,)
                idx_rac_1d = self._idx2[0]  # (N,)
                N = idx_csh_1d.numel()
                out = []
                used = torch.zeros(N, dtype=torch.bool, device=x.device)
                i = j = 0
                while i < N or j < N:
                    # push m from CSH
                    c = 0
                    while i < N and c < m:
                        ...
                        if not used[v]: used[v] = True; out.append(v); c += 1
                    # push n from RACS
                    c = 0
                    while j < N and c < n:
                ...
                # 补齐（理论上 out 已覆盖 N，但稳妥起见）
                if len(out) < N:
                    for v in range(N):
                        if not used[v]: out.append(v)
            
                idx_fused = torch.tensor(out, device=x.device, dtype=torch.long).view(1, -1).repeat(B, 1)  # (B,N)
                inv_fused = torch.argsort(idx_fused, dim=-1)                                              # (B,N)
            
                # 用 fused 索引重排 → SSM → 用 inv_fused 回折
                semantic_x = semantic_neighbor(x, idx_fused)      # (B,N,C)
